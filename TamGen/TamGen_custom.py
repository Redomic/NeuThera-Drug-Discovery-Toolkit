...
def prepare_pdb_data(pdb_id, ligand_inchi=None, DemoDataFolder="TamGen_Demo_Data", thr=10):
    ...
    script_path = os.path.abspath("scripts/build_data/prepare_pdb_ids.py")
    os.system(f"python {script_path} tmp_pdb.csv gen_{out_split} -o {DemoDataFolder} -t {thr}")
    # os.system(f"python TamGen/scripts/build_data/prepare_pdb_ids.py tmp_pdb.csv gen_{out_split} -o {DemoDataFolder} -t {thr}")
    os.remove("tmp_pdb.csv")

def prepare_pdb_data_center(pdb_id, ligand_inchi=None, DemoDataFolder="TamGen_Demo_Data", thr=10):
    out_split = pdb_id.lower()
    FF = glob(f"{DemoDataFolder}/*")
    for ff in FF:
        if f"gen_{out_split}" in ff:
    ...
    with open("tmp_pdb.csv", "w") as fw:
        print("pdb_id,ligand_inchi", file=fw)
        print(f"{pdb_id},{ligand_inchi if ligand_inchi else ''}", file=fw)
...
class TamGenCustom:
    ...
    def sample(self, m_sample=50, toolcompound=None, customer_filter_fn=None, maxseed=101):
        ...
        for seed in tqdm(range(1, maxseed), total=maxseed):
            ...
            with progress_bar.build_progress_bar(self.args, self.itr) as t:
                for sample in t:
                    sample = utils.move_to_cuda(sample) if False else sample  # Ensures CPU execution
                    sample = utils.apply_to_sample(lambda t: t.to(device), sample)  # Move tensors to CPU
